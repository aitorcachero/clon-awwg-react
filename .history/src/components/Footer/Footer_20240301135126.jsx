...
export default function Footer() {
  return (
    <footer
      className="lg:h-96 flex flex-col justify-center items-center gap-10 p-4"
    ...
    >
        ...
        {sections.map((section, i) => {
          return (
            <NavLink to={section.url} key={i}>
              <h2 className="text-white font-medium lg:text-xl">
                {section.name}
              </h2>
            </NavLink>
          );
      ...
      <section className="flex flex-col lg:flex-row gap-10 items-center">
        {footer.map((section, i) => {
          return (
            <a href="" key={i}>
              <h2 className="text-white text-xm  lg:text-lg">{section.name}</h2>
            </a>
          );
        })}
      </section>
